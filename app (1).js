  ...
  if (currentSubTab === 'leads') {
    ...
    main.innerHTML = formHtml + `<table class="data-table">
      ...
      ${leads.map(l => `<tr>
        ...
        <td>${l.arv ? `<button onclick="generateBrief(${l.lead_id})" style="padding:6px 12px;background:transparent;border:1px solid var(--gold-light);color:var(--gold-light);border-radius:6px;cursor:pointer;font-size:12px">Generate Brief</button>` : '<span style="color:var(--text-secondary);font-size:11px">Needs ARV</span>'}</td>
      </tr>`).join('')}
    </table>
